...
const getVolumeIcon = (volume) =>
  volumeIcons[Math.floor(volume * (volumeIcons.length - 1))]
...
const getSoundName = (sound) =>
  sound.currentMarker
    ? `"${sound.key}:${sound.currentMarker.name}"`
    : `"${sound.key}"`

const getSoundTimecode = (sound) =>
  `[${sound.seek.toFixed(1)} / ${sound.duration.toFixed(1)}]`
...
export class SoundWatcherPlugin extends Phaser.Plugins.BasePlugin {
  ...
  start() {
    ...
    if (!systemScene) {
      console.debug('waiting for system scene')
